...
# Example import statement in app.p
import  identity.web
...
def auth_response():
    ...
    # Redirect to process_candidate or any other endpoint after successful login
    return redirect(url_for("process_candidate"))
...
#     if request.endpoint not in ['login', 'authorized'] and not get_token_from_cache():
#         return redirect(url_for('login'))
...
def process_candidate():
    ...
    if returncode == 0:
        return jsonify({"message": f"Podman image started successfully with {num_candidates} candidates and batch size {batch_size}.", "output": output})
    else:
...
def stop():
    ...
    output, returncode = execute_podman_command(command)
    if returncode == 0:
        return redirect(url_for('stopped'))
    else:
...
#         else:
#             return "Authentication failed: " + result.get("error_description")
#     else:
